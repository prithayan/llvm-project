  ...
  for (auto Iter : MemUseToReachingDefsMap) {
    ...
    for (auto MemDefInstr : Iter.second) {
      ...
      if (EXISTSinMap(FuncEnvMap, DefFunc)) {
        ...
        // If both use and def on host or both in the samge target region, then
        // nothing to check.
...
OmpSanitizerGlobalAnalysis::Result
OmpSanitizerGlobalAnalysis::run(Module &M, ModuleAnalysisManager &AM) {
