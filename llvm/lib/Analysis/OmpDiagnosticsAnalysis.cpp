...
OmpDiagnosticsInfo &
OmpDiagnosticsInfo::operator=(OmpDiagnosticsInfo &&) = default;
...
int OmpDiagnosticsAnalysis::getSCEV_Max_value(
    std::vector<const SCEV *> &simplifySCEV, std::string &maxSCEV_string,
    const bool getMax) {
  if (simplifySCEV.empty())
    return 0;
  ...
  case scUMinExpr:
    assert(false && "scUMinExpr Not Handled");
  case scSMinExpr:
    assert(false && "scSMinExpr Not Handled");
  case scConstant: {
  ...
  case scAddRecExpr: {
    ...
    LLVM_DEBUG(dbgs() << "\n TRIP COUNT ::"
                      << SE->getSmallConstantMaxTripCount(fLoop)
...
OmpDiagnosticsInfo OmpDiagnosticsAnalysis::run(Function &F,
  ...
  // Iterate over all instructions with the basic block
  for (auto I = inst_begin(F), E = inst_end(F); I != E; I++) {
    Instruction &Instr = *I;
    // Check if the instruction is SCEV type,
    if (SE->isSCEVable(Instr.getType())) {
      LLVM_DEBUG(dbgs() << "\n Instruction::" << *I);
      auto Scev = SE->getSCEV(&Instr);
      if (isa<SCEVNAryExpr>(Scev)) {
        // Record the SCEV corresponding to the instruction.
        Instr2ScevMap[&Instr] = Scev;
        LLVM_DEBUG(dbgs() << "\n SCEV::" << *SE->getSCEV(&Instr));
      }
    }
    // Now handle all kinds of interesting instructions here, We are interested
    // in memory operations, so we should hanle store and load and call
    // instructions. Currently only store instruction is handled.
    if (auto St = dyn_cast<StoreInst>(&Instr)) {
      auto Address = St->getPointerOperand();
      // The index into a store instruction can be found by looking at the GEP
      // instruction.
      if (auto Gep = dyn_cast<GetElementPtrInst>(Address)) {
        // The gep instruction has multiple operators, but only one of them is
        // the actual index.
        for (unsigned OpNum = 0; OpNum < Gep->getNumOperands(); OpNum++) {
          if (OpNum == Gep->getPointerOperandIndex())
            continue;
          auto Index = Gep->getOperand(OpNum);
          // Next we need to find the actual definition of the index, that has a
          // proper SCEV, if it does not have a SCEV, then the loop index is not
          // interesting or not analyzable.
          while (Instr2ScevMap.find(Index) == Instr2ScevMap.end()) {
            // Keep on going to the def instruction, till we reach a non
            // instruction or we get the proper scev for the index.
            if (auto IndexInstr = dyn_cast<Instruction>(Index)) {
              // We need to handle different instruction kinds here
              Index = IndexInstr->getOperand(0);
            } else
              break;
          }
          // If this instruction has a SCEV type then it is an interesting index
          // op.
